...
public class CalibrationImageManager {

  public static CalibImageItem retrieveImage(String path) throws ImageProcessingException, IOException {
    Metadata metadata = ImageMetadataReader.readMetadata(new File(path));
    return new CalibImageItem(metadata, Paths.get(path).getFileName().toString(), new Image(new File(path).toURI().toString()), path);
  }
