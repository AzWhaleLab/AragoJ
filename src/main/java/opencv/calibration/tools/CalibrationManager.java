...
public class CalibrationManager {
    ...
    new Thread(() -> {
      ...
      for (int i = 0; i < imageCount && !stoppedCalibration; i++) {
        ...
        if (!cameraModel.isEmpty() && !extractedCameras.contains(cameraModel)) {
          ...
          if (listener != null) {
              listener.onProgress(i, imageCount,
                  Translator.getString("progressAnalyzingImage") + " " + imageItem.getName());
          }
      ...
      List<Mat> objectPoints =
          getObjectPoints(config.getHrzPoints(), config.getVertPoints(), imagePoints.size(), 1f);
      if (imagePoints.size() > 0) {
          if (listener != null && !stoppedCalibration) {
              listener.onProgress(imageCount, imageCount,
                  Translator.getString("progressCalibratingCamera"));
          }
        CalibrationResults calibrationResults =
            calibrateCamera(objectPoints, imagePoints, imageSize, config.getLens());
  ...
  private boolean findPattern(CalibrationConfig.Pattern pattern, Size boardSize, Mat grayFrame,
  ...
  private CalibrationResults calibrateCamera(List<Mat> objectPoints, List<Mat> imagePoints,
      Size imageSize, CalibrationConfig.Lens lens) {
  ...
  private List<Mat> getObjectPoints(int horizontalNr, int verticalNr, int listSize,
      float squareSize) {
