...
import com.aragoj.opencv.calibration.file.CalibrationFileManager;
import com.aragoj.opencv.calibration.model.CalibrationConfig;
import com.aragoj.opencv.calibration.model.CalibrationImage;
...
import com.aragoj.ui.custom.PixelatedImageView;
...
public class CalibrationDialogController implements ImageEditorStackGroup.ModeListener, CalibrationProgressDialogController.ProgressListener, CalibConfigDialogController.OnActionListener {
    ...
    public void init(Window owner) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fmxl/CalibrationDialog.fxml"), Translator.getBundle());
...
//            stage.setResizable(false);
    ...
    private void loadPreferences() {
        ...
        String sPattern = calibPrefs.get(Constants.CALIB_PATTERN, CalibrationConfig.Pattern.CHESSBOARD.toString());
        ...
        CalibrationConfig calibrationConfig = new CalibrationConfig(pattern, hrzPoints, vertPoints, 1, CalibrationConfig.Lens.fromString(toggle));
    ...
    public void onRunCalibration(ActionEvent actionEvent) throws InterruptedException, ExecutionException, IOException {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/fmxl/ProgressDialog.fxml"), Translator.getBundle());
        CalibrationProgressDialogController calibrationProgressDialogController = new CalibrationProgressDialogController(this);
        loader.setController(calibrationProgressDialogController);
        Parent root = loader.load();

        calibrationProgressDialogController.startCalibration(calibrationConfig, imageListView.getItems(), stackPane);
    ...
    public void onCalibrationOptions(ActionEvent actionEvent){
        ...
        calibConfigDialogController.init(imageEditorStackGroup.getScene().getWindow(), this);
    }
    ...
    public void onImportImages (ActionEvent actionEvent){
        ...
        if (files != null && files.size() > 0) {
            prefs.put(Constants.FILECHOOSER_CALIB_IMPORT_IMG_LASTOPENED, files.get(0).getParent());
            for (File file : files) {
                addImage(file);
            }
        }
    ...
    @Override
    public void onCalibrationResult(CalibrationResults results) {
        ...
        for (int i = 0; i < imageResults.size(); i++) {
            ...
            try {
                Image imageResultItem = imageResults.get(i).getImage();
                boolean found = imageResults.get(i).isFound();
                String color = "#C41E3A";
                if(found) color = "#00A550";
                imageItem = new ImageItem(listImageItem.getName(), listImageItem.getPath(), imageResultItem, listImageItem.getImageIcon(50), 50, color);
                CalibrationImageItem item = new CalibrationImageItem(imageEditorScrollPane);
        ...
        imageListView.getSelectionModel().select(selectedIndex);
    ...
    @Override
    public void onConfigurationSave(CalibrationConfig calibrationConfig) {
